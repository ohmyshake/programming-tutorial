...
- Storage, such as the hard disk on the server, is connected to the server (on the server) through a `SAS cable` (ultra-fast transmission speed, such as 10G/s). At this time, the read and write speed of the server is affected by the disk speed, and the general disk speed is only 1G/s s, but a server will be connected to multiple disks, as long as the transmission speed of multiple disks does not exceed 10G/s of the SAS line, reading and writing will not be affected. It can be seen from the above that the traditional hard disk storage method has defects. The speed of reading data is very slow, and the total bandwidth is constant. Many people read data at the same time, and the speed will slow down seriously; at the same time, if reading a large file, it can only be read serially. 
