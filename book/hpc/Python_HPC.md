...
- `Disk bound (small data files)`: same as network I/O, for example, you need to read many data files to do computing, the whole process is `send read data mission via CPU` --> `IO system reads data into memory` --> `do computing task`. When you read many small files, the `bandwidth` is not the limitation factor, and the main time is spent on other operations such as finding the address of the small file. So when IO system reads data into memory, your CPU is free at this time, you can use `multi-threads` to enhance the efficiency.

- `Disk bound (big data files)`: single thread or process is well. However, when reading large files, the limitation at this time is limited by the disk speed and bandwidth, and the multi-threaded reading will not significantly increase the speed. So many processes for IO reading? This is not possible. At this time, the resource time of multiple processes is spent on context switching, because the CPU has time slicing and runs different processes at different times. When a process is suspended, it needs to record the state at this time. When a process resumes, it needs to collect the state of the previous moment and then execute the program.
